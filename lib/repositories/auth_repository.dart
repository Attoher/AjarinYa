...
abstract class AuthRepository {
  ...
  Future<void> updateGroupId(String groupId); // keep for backward compat internally if needed
...
class AuthRepositoryImpl implements AuthRepository {
    ...
    if (db != null) {
      try {
        ...
        if (docSnapshot.exists && docSnapshot.data() != null) {
          developer.log('User profile found in Firestore for uid: $uid', name: 'AUTH_DIAGNOSTICS');
          return UserProfile.fromJson(docSnapshot.data()!);
        } else {
          developer.log('User profile NOT found in Firestore. Creating default for uid: $uid', name: 'AUTH_DIAGNOSTICS');
        ...
        developer.log('Error loading/creating profile in Firestore: $e', name: 'AUTH_DIAGNOSTICS');
      }
    }
  ...
  Future<UserProfile?> loginWithEmailAndPassword(String email, String password) async {
    ...
    if (firebaseAuth != null) {
      try {
        ...
        developer.log('Firebase Auth login gagal: $e', name: 'AUTH_DIAGNOSTICS');
        rethrow;
      }
    }
  ...
  Future<UserProfile?> registerWithEmailAndPassword(
    ...
    if (firebaseAuth != null) {
      try {
        ...
        developer.log('Firebase Auth register gagal: $e', name: 'AUTH_DIAGNOSTICS');
        rethrow;
      }
    }
    throw Exception('Firebase Auth tidak terinisialisasi.');
  ...
  Future<void> joinGroup(String groupId, {String? groupName}) async {
    if (_currentCachedUser != null) {
      ...
      final currentNames = Map<String, String>.from(_currentCachedUser!.groupNames);
      ...
      if (db != null) {
        try {
          ...
          await db.collection('users').doc(updatedProfile.uid).set(updatedProfile.toJson());
          developer.log('Persisted user profile to Firestore for joinGroup', name: 'AUTH_DIAGNOSTICS');
        } catch (e) {
          developer.log('Failed to persist user profile: $e', name: 'AUTH_DIAGNOSTICS');
        }
      }
    ...
    if (_currentCachedUser != null) {
      ...
      if (db != null) {
        try {
          ...
          await db.collection('users').doc(updatedProfile.uid).set(updatedProfile.toJson());
          developer.log('Persisted user profile to Firestore for leaveGroup', name: 'AUTH_DIAGNOSTICS');
        } catch (e) {
          developer.log('Failed to persist user profile: $e', name: 'AUTH_DIAGNOSTICS');
        }
      }
    ...
    if (_currentCachedUser != null) {
      if (_currentCachedUser!.groupIds.contains(groupId)) {
        ...
        if (db != null) {
          try {
            await db.collection('users').doc(updatedProfile.uid).set(updatedProfile.toJson());
            developer.log('Persisted user profile to Firestore for switchActiveGroup', name: 'AUTH_DIAGNOSTICS');
          } catch (e) {
            developer.log('Failed to persist user profile: $e', name: 'AUTH_DIAGNOSTICS');
          }
        }
