...
| Team Member | Feature Module | Screens & Components | ViewModel / Repository | Cloud Backend |
|---|---|---|---|---|
| Rafif Thariq (5025231213) | Question Forum | `QuestionForumScreen` (list soal, filter tag, upvote), `AnswerQuestionScreen` (tampil soal + jawab) | `QuestionViewModel.createQuestion()`, `addReply()`, `toggleBestReply()`, `toggleSolved()`; `QuestionRepository` + `QuestionRepositoryImpl` | Firestore `questions` collection (buat soal, baca stream, update isBest/isSolved, hapus soal & jawaban) |
| Ath Thahir Muhammad Isa Rahmatullah (5025231181) | Skill Barter | `BarterRequestScreen` (list tawaran, form buat tawaran, tombol Apply) | `BarterViewModel` (`createBarterRequest`, `applyBarter`, `deleteBarterRequest`); `BarterRepository` + `BarterRepositoryImpl` (Firestore transaction ATOMIC match anti race-condition) | Firestore `barter_requests` collection; status PENDING -> MATCHED via `runTransaction` |
| Ath Thahir Muhammad Isa Rahmatullah (5025231181) | Private Chat | `PrivateChatScreen` (bubble chat, kirim teks & gambar, header info partner) | `ChatViewModel` (`sendTextMessage`, `sendImageMessage`, `subscribeToMessages`); `ChatRepository` + `ChatRepositoryImpl`; `SupabaseStorageService.uploadChatImage()` | Firestore `barter_requests/{id}/messages` sub-collection (real-time stream); Supabase Storage bucket `Chat` |
| Ath Thahir Muhammad Isa Rahmatullah (5025231181) | Forum Reply & Comment | `_ReplyCard` (StatefulWidget: expand/collapse komentar, input komentar, upload gambar); `_CommentTile` (tampil komentar + hapus) | `QuestionViewModel.addComment()`, `deleteComment()`; `SupabaseStorageService.uploadCommentImage()` | Firestore `questions/{id}` — komentar disimpan nested di dalam array `replies[].comments[]` |
| Ath Thahir Muhammad Isa Rahmatullah (5025231181) | Study Spot Explorer | `StudySpotScreen` (peta interaktif flutter_map, marker lokasi, form tambah spot + foto) | `StudySpotViewModel` (`fetchStudySpots`, `createStudySpot`, `updateStudySpot`, `deleteStudySpot`); `StudySpotRepository` + `StudySpotRepositoryImpl`; `SupabaseStorageService.uploadStudySpotImage()` | Firestore `study_spots` collection (GeoPoint koordinat); Supabase Storage bucket `StudySpots` |
| Dzuhrillah Hendraines (5025221107) | Notes Collection | `NotesCollectionScreen` (grid catatan per folder, warna label, bookmark); `NoteEditorScreen` (buat/edit catatan + lampiran gambar) | `NotesViewModel` (`loadNotes`, `createNote`, `updateNote`, `deleteNote`, `toggleBookmark`); `NotesRepository` + `NotesRepositoryImpl` | Firestore `notes` collection (filter per `ownerId`) |
| Dzuhrillah Hendraines (5025221107) | Pomodoro Timer | `PomodoroScreen` (countdown fokus/istirahat, simulasi ruang belajar online, musik latar) | State lokal `StatefulWidget` (timer countdown, mode fokus/break, kontrol musik) | — (fitur lokal, tidak ada penyimpanan cloud) |
| Ath Thahir Muhammad Isa Rahmatullah (5025231181) | Push Notification | `NotificationService` (izin, tampil notifikasi sistem foreground via `flutter_local_notifications`, navigasi tap); `firebaseMessagingBackgroundHandler` (top-level isolate handler) | `AuthViewModel._syncFcmToken()` -> `AuthRepository.updateFcmToken()` -> Firestore `users/{uid}.fcmToken` | Cloud Functions `functions/index.js`: `sendChatMessageNotification`, `sendBarterOfferNotification`, `sendBarterMatchedNotification` (FCM Admin SDK) |
